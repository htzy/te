...
<!-- 导入框架必须文件开始 -->
<base href="<%=basePath%>">
<title>SensorTypeOperation</title>
...
<link href="<%=basePath%>libs/css/showAllSensorType.css"
...
<body>
	...
	<%!// 从servlet接受到的数据，数据结构：{SensorTypeID，SensorTypeName}交替存储在str字符串数组中
	String[] str;%>
	<%!HttpSession session;%>
	...
	<div class="box2" panelTitle="操作">
		...
		<div class="halfleft">
			<div align="left">
				<form action="<%=basePath%>servlet/SearchSensorTypeServlet" id="sensorTypeForm"
					target="frmright" method="post">
					<div class="box2" panelTitle="操作">
						<input type="hidden" id="SensorTypeID" name="SensorTypeID"
							value="" />
						<div style="font-size:20px">
							传感器类型 <input type="text" id="SensorTypeName" class="validate[required,custom[illegalLetter]]"
								name="SensorTypeName" value="" /> <input type="submit" id="ch"
								value="查找" onclick="return searchSensorType();" />
						</div>
					</div>
				</form>
			</div>
		...
		<!-- 居左显示查找条件输入框和查找按钮结束 -->
		<!-- 右中显示增加按钮开始 -->
		<div class="halfright hand" onclick="addSensorType()">
			<div align="center">
				<div>
	...
	<div id="scrollContent" style="overflow-x:hidden;">
		<!-- 显示SensorType详细信息，表头开始 -->
		<table class="tableStyle" mode="list">
			<tr>
				<th width="50%">传感器类型</th>
				<th width="25%">修改</th>
				<th width="25%">删除</th>
			</tr>
		</table>
		<!-- 显示SensorType详细信息，表头结束 -->

		<!-- 显示SensorType详细信息，循环分条显示详细信息开始 -->
		<%
			for (; j < length && i < end; i += 2, j += 2) {
		%>
		<form action="<%=basePath%>servlet/ChangeSensorTypeServlet"
			...
			<div>
				...
				<table class="tableStyle" mode="list">
					<tr>
						<td width="50%"><div>
								<input type="text" style="width:100%;" id="SensorTypeName"
									class="validate[required,custom[illegalLetter]]" name="SensorTypeName"
									value="<%=str[j]%>"
									onkeydown="javascript:if(event.keyCode=='13')return false;" />
							</div>
						</td>
						<td width="25%"><span id="changeSensorType<%=i%>"
							class="img_edit hand" title="修改"
							onclick="changeSensorType(<%=i%>);"></span></td>
						<td width="25%"><span id="delete<%=i%>"
							class="img_delete hand" title="删除"
							onclick="delSensorType(<%=str[i]%>)"></span>
						</td>
					</tr>
				</table>
			</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			$("#page").bind("pageChange",function(e,index){
				window.location.href="<%=basePath%>jsp/sensorType/showAllSensorType.jsp?page="+index;
			});
		});
		
		function validateForm(containerId1) {
			var valid1 = $("#sensorTypeForm").validationEngine({
				returnIsValid : true
			});
			return valid1;
		} 
		...
		*/
		...
		/**
		 * 批量增加SensorType时，跳转到addSensorType.jsp页面
		 */
		function addSensorType(){
			var diag = new top.Dialog();
			diag.Title = "增加";
			diag.URL = "<%=basePath%>jsp/sensorType/popup/addSensorType.jsp";
			diag.show();
		}
		
		/**
		 * 点击查找时，根据输入的内容SensorTypeName进行查找并显示在此页面
		 */
		function searchSensorType(){
			if(document.getElementById("SensorTypeName").value==""){
				return false;
			}else{
				return true;
			}
		 ...
		 * 删除时提示是否确认删除
		 */
		function delSensorType(SensorTypeID){
			if(confirm("真的要删除吗？")){
       			location.href="<%=basePath%>servlet/DelSensorTypeServlet?SensorTypeID=" + SensorTypeID;
			}else{
			}
		}
