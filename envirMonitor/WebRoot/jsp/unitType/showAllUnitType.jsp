...
<link href="<%=basePath%>libs/css/showAllUnitType.css" rel="stylesheet"
	type="text/css" />
...
<body>
	<%!// 从servlet接受到的数据，数据结构：{unitTypeID，unitTypeName}交替存储在str字符串数组中
	String[] str;%>
	<%!// i用于循环unitTypeID，j用于循环unitTypeName
	int i = 0, j = 1, end;%>
	<%!// str的长度
	int length = 0;%>
	<%!// pag用于分页的记录当前页面是第几页
	...
	<div class="box2" panelTitle="操作">
		...
		<div class="halfleft">
			<div align="left">
				<form action="<%=basePath%>servlet/SearchUnitTypeServlet"
					target="frmright" method="post">
					<div class="box2" panelTitle="操作">
						<input type="hidden" id="unitTypeID" name="unitTypeID" value="" />
						<div style="font-size:20px">
							单元类型 <input type="text" id="unitTypeName" name="unitTypeName"
								value="" /> <input type="submit" id="ch" value="查找"
								onclick="return searchUnitType();" />
						</div>
					</div>
				</form>
			</div>
		...
		<!-- 居左显示查找条件输入框和查找按钮结束 -->
		<!-- 右中显示增加按钮开始 -->
		<div class="halfright hand" onclick="addUnitType()">
			<div align="center">
				<div>
	...
	<div id="scrollContent" style="overflow-x:hidden;">
		...
		%>
		<form action="<%=basePath%>servlet/ChangeUnitTypeServlet"
			id="changeForm<%=i%>" target="frmright" method="post">
			<div>
				<input type="hidden" id="unitTypeID" name="unitTypeID"
					value="<%=str[i]%>" />
				<table class="tableStyle" mode="list">
					<tr>
						<td width="50%"><div>
								<input type="text" style="width:100%;" id="unitTypeName"
									class="validate[required] " name="unitTypeName"
									value="<%=str[j]%>"
									onkeydown="javascript:if(event.keyCode=='13')return false;" />
							</div>
						</td>
						<td width="25%"><span id="changeUnitType<%=i%>"
							class="img_edit hand" title="修改"
							onclick="changeUnitType(<%=i%>);"></span></td>
						<td width="25%"><span id="delete<%=i%>"
							class="img_delete hand" title="删除"
							onclick="delUnitType(<%=str[i]%>)"></span>
						</td>
					</tr>
				</table>
			</div>
	...
	<div class="float_right padding5">
		<div class="pageNumber" total="<%=length / 2%>" showInput="true" page="<%=pag %>"
	...
	<script type="text/javascript">
		...
		*/
		...
		function addUnitType(){
			...
			diag.Title = "增加";
			diag.URL = "<%=basePath%>jsp/unitType/popup/addUnitType.jsp";
			diag.show();
		}
		 ...
		 * 删除时提示是否确认删除
		 */
		function delUnitType(unitTypeID){
			if(confirm("真的要删除吗？")){
       			location.href="<%=basePath%>servlet/DelUnitTypeServlet?unitTypeID=" + unitTypeID;
			}else{
			}
		}
		...
		$(function(){
			$("#page").bind("pageChange",function(e,index){
				window.location.href="<%=basePath%>jsp/unitType/showAllUnitType.jsp?page="+index;
			});
		});
	</script>
</body>
