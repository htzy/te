...
<link href="<%=basePath%>libs/css/showAllSensorType.css" rel="stylesheet"
	type="text/css" />
...
<body>
	...
	<form action="<%=basePath%>servlet/AddSensorTypeServlet" id="addForm"
		target="frmright" method="post">
		<input type="hidden" id="SensorTypeNames" name="SensorTypeNames" value="" />
		<div class="box1" whiteBg="true">
			<div align="center">
				<input type="button" value="添加" onclick="addHandler()" />
				<input type="button" id="add" name="add" value="保存" onclick="addSensorType()" />
	...
	<script>
		...
		function validateForm(containerId1) {
			var valid1 = $("#SensorTypeNameForm").validationEngine({
				returnIsValid : true
			});
			return valid1;
		}
		...
		function addNewRowHandler() {
			var $tr = $("<tr><td>"
					+ "<input style='width:100%;' id='SensorTypename"
					+ sum
		...
		function addSensorType() {
			//检查是否输入非法字符
			var flag = validateForm("#SensorTypeNameForm");
			if (flag == false) {
			...
			var temp = "";
			for ( var i = 0; i < sum; i++) {
				temp = document.getElementById("SensorTypename" + i);
				if (temp != null) {
					if (temp.value == "") {
						alert("还有未填项，请检查重试！");
						return;
					}
					SensorTypeNames += temp.value + ",";
				}
			}
			...
			$("#SensorTypeNames").val(SensorTypeNames);
