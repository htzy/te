...
<body>
	...
	<form action="<%=basePath%>servlet/AddUnitTypeServlet" id="addForm"
		target="frmright" method="post">
		<input type="hidden" id="unitTypeNames" name="unitTypeNames" value="" />
		<div class="box1" whiteBg="true">
			<div align="center">
				<input type="button" value="添加" onclick="addHandler()" />
				<input type="button" id="add" name="add" value="保存" onclick="addUnitType()" />
	...
	<script>
		...
		function addNewRowHandler() {
			var $tr = $("<tr><td>"
					+ "<input style='width:100%;' id='unittypename"
					+ sum
					+ "' class='validate[required,custom[illegalLetter]]' name='unittypename"
					+ sum
					+ "' "
					+ "value='' />"
		...
		function addUnitType() {
			//检查是否输入非法字符
			var flag = validateForm("#unitTypeNameForm");
			if (flag == false) {
			...
			var temp = "";
			for ( var i = 0; i < sum; i++) {
				temp = document.getElementById("unittypename" + i);
				if (temp != null) {
					if (temp.value == "") {
						alert("还有未填项，请检查重试！");
						return;
					}
					unitTypeNames += temp.value + ",";
				}
			}
			...
			$("#unitTypeNames").val(unitTypeNames);
